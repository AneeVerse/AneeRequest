        ...
        <div className={`flex-1 flex flex-col min-w-0 bg-[#101011] rounded-t-2xl overflow-hidden border-t border-l border-r mt-2 sm:mt-6 responsive-content-wrapper transition-all duration-500 ${isImpersonating ? 'border-[#22c55e]/60 shadow-[0_0_15px_rgba(34,197,94,0.15)]' : 'border-shark'}`}>
          <div className="border-b border-shark">
            <Header
              ...
              label={<span className="hidden sm:inline">DRIVE</span>}
              labelIcon={<FolderOpen size={20} className="text-[#279da6] hidden sm:block" />}
                              ...
                              >
                                ...
                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  ...
                                  <button
                                    ...
                                    className={`p-1.5 transition-all ${copiedId === item.id ? 'text-emerald-400 scale-110' : 'hover:text-[#279da6]'}`}
                        ...
                        <div key={item.id}>
                          ...
                          >
                            ...
                            <div className="flex items-center justify-end gap-2 pt-3 border-t border-shark/30">
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); handleCopyLink(item.id, item.webViewLink); }}
                                className={`p-2 rounded-xl border transition-all ${copiedId === item.id ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400' : 'bg-shark/40 border-shark/60 text-storm-gray hover:text-[#279da6]'}`}
                                title={copiedId === item.id ? "Copied!" : "Copy Link"}
                              >
                                {copiedId === item.id ? <Check size={18} /> : <Link size={18} />}
                              </button>
                          ...
                          >
                            ...
                            <div className="w-32 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); handleCopyLink(item.id, item.webViewLink); }}
                                className={`p-1.5 rounded-lg transition-all ${copiedId === item.id ? 'bg-emerald-500/20 text-emerald-400' : 'hover:bg-[#279da6]/20 text-storm-gray hover:text-[#279da6]'}`}
                                title={copiedId === item.id ? "Copied!" : "Copy Link"}
                              >
                                {copiedId === item.id ? <Check size={18} /> : <Link size={18} />}
                              </button>
